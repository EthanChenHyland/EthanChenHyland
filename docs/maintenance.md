...
The ASCII hero reveals once, then breathes and makes a small hop on a seven-second loop. Only the artwork transforms; the SVG dimensions remain fixed. Reduced-motion preferences disable both the reveal and idle loop. The animation is regenerated by `scripts/make_ascii.py`.
